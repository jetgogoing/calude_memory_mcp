#!/usr/bin/env python3
"""
测试脚本：使用已配置的OpenRouter API生成包含Eric相关信息的文章并注入Claude Memory系统
"""
...
async def inject_memory_with_openrouter(content: str, project_id: str = "shenrui_investment"):
    """使用OpenRouter API将内容注入到Claude Memory系统"""
    
    print(f"正在初始化服务管理器...")
    
    # 临时修改设置，使用OpenRouter的模型
    settings = get_settings()
    # 修改默认的轻量模型为OpenRouter支持的模型
    settings.models.default_light_model = "deepseek/deepseek-chat-v3-0324"
    settings.memory.fuser_model = "deepseek/deepseek-chat-v3-0324"
    ...
    try:
        ...
        # 先确保项目存在
        from claude_memory.managers.project_manager import ProjectManager
        project_manager = ProjectManager(service_manager.session_manager)
        
        # 创建或激活项目
        await project_manager.create_or_update_project(
            project_id,
            name="燊锐投资研究院",
            description="燊锐投资研究院的记忆存储项目"
        )
        
        cross_project_request = CrossProjectSearchRequest(
            query=search_query,
            project_ids=[project_id, "default"],  # 搜索多个项目
        ...
        for project_id, project_result in cross_search_response.project_results.items():
            ...
            if project_result.results:
                for j, result in enumerate(project_result.results[:2]):  # 只显示前2个结果
                    print(f"  结果 {j+1}: {result.memory_unit.title}")
...
async def main():
    """主函数"""
    print("=== Claude Memory Eric 信息注入测试（使用OpenRouter） ===\n")
    ...
    conversation_id = await inject_memory_with_openrouter(content)
    ...
    if conversation_id:
        ...
        print(f"      mcp__claude-memory__claude_memory_search('Eric', project_id='shenrui_investment')")
